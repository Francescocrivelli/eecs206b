...
class ExtendedKalmanFilter(Estimator):                      # THIS PART IS THE EXTRA CREDIT     
    ...
    def __init__(self):
        ...
        # Define noise covariance matrices
        # self.Q = np.diag([0.001, 0.01, 0.01, 0.001, 0.01]) * 0.1 # Process noise covariance                [phi, x, y, thetaL, thetaR]    
        # self.Q = np.diag([0.01, 0.1, 0.1, 0.01, 0.1]) * 0.1     
        self.Q = np.array([
        ...
        # self.R = np.diag([0.1, 1]) * 0.01 # Measurement noise covariance   
        # self.R = np.diag([0.1, 1]) * 0.01                       
        self.R = np.array([
           [0.001, 0],           
           [0, 0.001]])
        # self.P = np.diag([1, 10, 10, 1, 1]) * 1  # Initial state covariance  
        # self.P = np.diag([1, 10, 10, 1, 1]) * 1                
        self.P = np.array([[0.1, 0, 0, 0, 0],            # phi uncertainty and correlations
                           [0, 10, 0, 0, 0],             # x uncertainty and correlations
                           [0, 0.01, 0, 0, 0],             # y uncertainty and correlations
                           [0, 0, 0, 1, 0],                 # theta_L uncertainty
                           [0, 0, 0, 0, 1]])             # theta_R uncertainty
        #rospy.init_node('ekf_turtlebot_controller', anonymous=True)
        self.vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10)

    def publish_velocity(self, u_L, u_R):
        cmd = Twist()
        r,d = self.r, self.d
        print(f"U_L = {u_L}, U_R = {u_R}")

        cmd.linear.x = (r/2) * (u_L + u_R)
        cmd.angular.z = (r / (2 * d)) * (u_R - u_L)

        self.vel_pub.publish(cmd)
        rospy.loginfo(f"Pubished velocity: linear = {cmd.linear.x}, angular = {cmd.angular.z}")
    ...
    def g(self, x, u):
        ...
        x_pos_new = x_pos - (self.r * 0.5 * phi *np.sin(phi)) * (u_L + u_R) * self.dt + (self.r / 2) * np.cos(phi) * (u_L + u_R) * self.dt
        y_pos_new = y_pos   + (self.r * 0.5 * phi *np.cos(phi)) * (u_L + u_R) * self.dt + (self.r / 2) * np.sin(phi) * (u_L + u_R) * self.dt
    ...
    def approx_A(self, x, u):
        ...
        # return A
        # A = np.zeros((5,5))
        A[1,0] = -(self.r * 0.5 * (np.sin(phi) + phi * np.cos(phi))) * (u_L + u_R) * self.dt + (self.r * 0.5 * np.cos(phi)) * (u_L + u_R) * self.dt
        A[2,0] = (self.r * 0.5 * (np.cos(phi) - phi * np.sin(phi))) * (u_L + u_R) * self.dt + (self.r * 0.5 * np.cos(phi)) * (u_L + u_R) * self.dt
        return A
