...
class Environment:
    def __init__(self, dynamics, controller, landmark, observer = None, is_noise = True):   # Changed is_noise from False to True
        """
        ...
        self.w = 0.05
        # self.w = np.clip(0.5 * (np.random.normal() + 1), 0, 1)
        self.v = 0.1
        # self.v = np.clip(0.5 * (np.random.normal() + 1), 0, 1)
    ...
    def run(self):
        ...
        return self.xHist, self.uHist, self.tHist, self.obsHistimport numpy as np
import time
